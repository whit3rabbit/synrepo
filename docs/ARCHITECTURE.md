...
- `Config.max_graph_snapshot_bytes` defaults to `128 MiB`. Oversized snapshots are not published, so readers fall back to the SQLite path. Setting it to `0` disables publication entirely.
