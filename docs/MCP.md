...
Failed-tool Sentry telemetry is off by default and requires both `mcp_sentry_telemetry = true` in the merged synrepo config and `SYNREPO_SENTRY_DSN` set on the MCP process. The dashboard Actions tab exposes `O` to write the repo-local opt-in or opt-out policy; the DSN stays in process environment and should not be stored in repo files. The event payload is deliberately narrow: a fixed failure message and bounded tags for `component`, `tool`, `error_code`, `version`, `os`, and `arch`. It does not send repository paths, targets, queries, prompts, responses, error messages, user/request data, breadcrumbs, stacktraces, release/environment values, or Sentry default integrations. Empty or invalid `SYNREPO_SENTRY_DSN` disables telemetry for that process.
