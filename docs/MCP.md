...
Framework route bindings are graph facts when the parser observes clear framework syntax. Current extraction covers Rust Axum/Actix/Rocket-style routes, Python FastAPI/Flask/Django URL declarations, TypeScript/JavaScript Express and NestJS HTTP controllers, Java/Kotlin Spring mappings, Go Gin/chi/gorilla-style handlers, ASP.NET attributes, Laravel routes/resources, and Rails routes/resources. Route bindings are represented as `SymbolKind::Route` symbols and `References` edges to resolved handler symbols. They are parser-observed facts, not inferred overlay content.
