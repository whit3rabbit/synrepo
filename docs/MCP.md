...
`synrepo mcp` serves repository context to MCP-compatible coding agents over stdio. It is the agent-facing delivery surface for graph-backed code artifacts, task contexts, cards, search, impact analysis, test discovery, advisory overlay data, explicit saved repo lessons, optional saved-context note writes, and optional anchored source edits.
...
Failed-tool Sentry telemetry is off by default and requires `mcp_sentry_telemetry = true` in the merged synrepo config. The dashboard Actions tab exposes `O` to write the repo-local opt-in or opt-out policy. When enabled, MCP uses the built-in Sentry project DSN unless `SYNREPO_SENTRY_DSN` is set on the MCP process; the env value takes priority for custom routing. Empty or invalid `SYNREPO_SENTRY_DSN` disables telemetry for that process instead of falling back. The event payload is deliberately narrow: a fixed failure message and bounded tags for `component`, `tool`, `error_code`, `version`, `os`, and `arch`. It does not send repository paths, targets, queries, prompts, responses, error messages, user/request data, breadcrumbs, stacktraces, release/environment values, or Sentry default integrations.

Most users should prefer `synrepo setup <tool>`, which writes the agent instructions or skill and registers MCP through `agent-config` for supported integrations. The default is global agent config with `synrepo mcp`; pass `--project` to write repo-local MCP config that launches `synrepo mcp --repo .`. Repo-local setup follows the `agent-config` registry for every local MCP, skill, or instruction surface it supports, and `synrepo remove [tool] --apply` removes those owned entries through the same ownership ledger. Legacy unowned `synrepo` entries are still detected and removed with a warning for compatibility. Global MCP is lazy: each tool call must supply a registered repository via `repo_root` unless the server has a default repository. Agents can call `synrepo_use_project` once to set a session default for registered projects. Shim-only integrations still need their own MCP config pointed at `synrepo mcp --repo .`.

Codex and Claude can also install local client-side nudges with `synrepo setup codex --agent-hooks` or `synrepo setup claude --agent-hooks`. These hooks call `synrepo agent-hook nudge`, remind the agent to use synrepo before direct grep/read/review/edit workflows, and never block tools or store prompt content. Read hooks may add advisory read-cost and repeated-read hints from bounded state in `.synrepo/state/agent-hook-reads.json`; that state stores only repo-relative path, file size, mtime, timestamps, read count, and estimated tokens, expires after 8 hours, and is capped at 256 entries. Successful installs are tracked in `~/.synrepo/projects.toml`; `synrepo remove` and `synrepo uninstall` strip only the synrepo nudge commands from the JSON hook config and preserve unrelated user hooks/settings.
...
Use `tiny` budgets to route, `normal` budgets to understand a neighborhood, and `deep` budgets only before implementation or when exact source details matter. Use `synrepo_readiness` for cheap operational health, `synrepo_task_route` for cheap route classification, and `synrepo_overview` only when the full dashboard is useful. Use `synrepo_context_pack` when batching several known read-only code artifacts or task-context pieces is cheaper than serial tool calls. Its `targets` parameter is an array of structured objects: `{ "kind": "file|symbol|source_slice|directory|minimum_context|test_surface|call_path|search|entrypoints|public_api|change_risk|findings|recent_activity", "target": "...", "budget": "tiny|normal|deep" }`. `source_slice` returns line-numbered clustered source for a file or symbol only when the live file hash still matches the graph hash; stale files are omitted with a reconcile note instead of returning untrusted source. For task-context artifacts, `entrypoints` uses `target: "."` for whole-repo scope or a path prefix, `public_api` uses a directory path, `change_risk` uses a file path or symbol, `findings` uses `target: "all"` for bounded unfiltered overlay audit findings, and `recent_activity` uses `target: "release_readiness"` for release-relevant operational activity.

`synrepo_ask` accepts `ask`, optional `scope: { paths, symbols, change_set }`, optional `shape.sections`, optional `ground: { mode|citations, include_spans, allow_overlay }`, and optional `budget: { max_tokens, max_files, max_symbols, freshness, tier }`. Structured `ground.mode` and `ground.citations` values are exactly `required`, `preferred`, and `off`; do not pass `observed` there because `observed` is only an evidence confidence label. For compatibility with shorthand callers, `scope` may also be a whitespace-separated path string, `shape` may be a string or string array, `ground` may be a policy string such as `observed graph/source only`, and `budget` may be a tier string such as `normal`. It deterministically infers a built-in recipe such as `explain_symbol`, `trace_call`, `review_module`, `security_review`, `release_readiness`, or `fix_test`, compiles it into existing context-pack targets, and returns `{ answer, cards_used, evidence, grounding, omitted_context_notes, next_best_tools, context_packet }`. Evidence entries include `claim`, `source`, `span`, `spans`, `source_store`, `confidence`, and `provenance`, where `span` is `null` and `spans` is empty when line spans are unavailable or `ground.include_spans=false`. Empty-scope asks promote up to three high-confidence lexical hits into concrete file artifacts before broad search artifacts, prefer live `src/` and `tests/` paths, exclude benchmark fixture self-hits, and downrank archived specs when live source exists. Recipes compose existing deterministic cards, source slices, and searches: symbol explanation adds symbol, source-slice, minimum-context, and call-path artifacts; trace uses call paths, source slices, and entrypoints; module review uses module, public API, entrypoints, scoped risk, source slices, and lifecycle/error probes for broad reviews; security review adds risky-flow lexical probes; release readiness adds recent activity and, only when overlay is allowed, findings; fix-test adds test surface and target context. It is read-only and does not let LLM output mutate graph facts.
...
`synrepo_find` and `synrepo_where_to_edit` route plain-language tasks to tiny file cards. They are best for task routing, not broad review synthesis and not exact code symbols, string literals, flags, schema fields, tool names, or file paths. They first try the task text as-is, then use bounded deterministic fallback queries over code-shaped tokens, camel/snake/dot identifier parts, acronyms, light stems, known domain anchors, phrase, token, and snake_case variants. Repeated hits across fallback queries boost a file, while test, fixture, example, and spec paths are downranked for routing diversity. Responses include `query_attempts`, `fallback_used`, `miss_reason` (`no_index_matches` or `matches_not_in_graph`), and explicit `omitted` entries when the default internal cap drops suggestions. On misses, responses may include `recommended_next_queries` and `recommended_tool: "synrepo_search"` to turn broad failures into exact follow-up probes. If the task is broad architecture or review work, call `synrepo_ask` first. If the task includes exact identifiers, call `synrepo_search` first.

`synrepo_search` is read-only search backed by the syntext substrate index for the primary checkout, per-root syntext indexes for configured branch refs, and a bounded direct scan for other discovered non-primary roots such as linked worktrees. Exact path-shaped queries also return discovered repo files even when the path itself is not graph-backed. In `mode: "auto"`, it also uses the local vector index when semantic triage is available. It is the first choice for exact symbols, string literals, CLI flags, MCP tool names, schema keys, file paths, and code-review validation. It accepts `query`, optional `literal` (`false` default), optional `mode` (`auto` default, or `lexical`), optional `limit` (default `10`, capped at `50`, with `0` clamped to `1`), optional `path_filter`, optional `file_type`, optional `exclude_type`, optional `case_insensitive` (`ignore_case` is accepted as an alias), optional `output_mode` (`compact` default, `default`, or `cards`), and optional `budget_tokens` (default `1500` for compact or cards output). When `literal: true`, regex metacharacters are escaped before search. When `literal` is false and the pattern fails to compile, search retries as an escaped literal and returns `pattern_mode: "literal_fallback"` with `warnings`; normal regex and explicit literal responses return `pattern_mode: "regex"` or `"literal"`. Compact default responses group matches by `(root_id, path)` and include `suggested_card_targets`, root-aware `suggested_card_requests`, `omitted`, and `output_accounting`. Explicit `output_mode: "default"` returns bounded raw rows as `results: [{ path, root_id, is_primary_root, root_kind?, root_label?, root_ref?, root_commit?, editable?, file_id?, line, content, source, fusion_score, semantic_score?, chunk_id?, symbol_id? }]`. Branch-ref rows set `root_kind: "branch_ref"`, include the configured `root_ref`, report the observed `root_commit`, and set `editable: false`. Auto mode fuses lexical and semantic candidates when possible; otherwise it is an explicit lexical fallback with `semantic_available: false`. Semantic-only rows may have `path`, `line`, and `content` as null when the vector row cannot be graph-enriched. Pass `mode: "lexical"` to force exact lexical behavior.

Use compact output for broad searches where routing matters more than raw snippets. Adaptive compact search returns the smallest useful shape: grouped file previews for broad matches, a minimal miss object for zero results, or bounded raw rows for tiny result sets when raw is smaller. Every adaptive compact response includes `output_accounting`; `estimated_tokens_saved` stays at zero whenever the returned shape is not smaller than the original raw rows. Use `output_mode: "cards"` only for narrow searches: effective `limit <= 5` or a `path_filter`. Cards mode dedupes matched graph files, returns tiny file cards, reports unresolved paths, and applies the shared card-set cap when `budget_tokens` is present. `synrepo_card` returns a `module_card` for existing directory targets, and a bounded `filesystem_fallback` card with `graph_backed: false` for existing repo text files that are not present in the graph. Graph-only impact/risk tools return `INVALID_PARAMETER` with a not-graph-backed message when the target exists on disk but cannot produce graph impact. Multi-target card batches default to a 4000-token internal cap and report omitted targets instead of relying on final JSON truncation. `synrepo_context_pack` also accepts `output_mode: "compact"`; only search artifacts use the adaptive compact behavior, while card artifacts retain their existing `context_accounting`.

MCP search is read-only. It searches the persisted substrate indexes as-is and does not reconcile, rebuild indexes, download semantic models, start watch, or silently refresh state. Use `synrepo watch`, `synrepo reconcile`, `synrepo sync`, or initialization flows when you want structural and lexical indexes updated after source changes; use `synrepo embeddings build` for the first semantic vector build. When watch auto-sync is enabled, source-changing reconciles may refresh an existing vector index in the background after the repo quiets down. Clients should treat `semantic_available` and `routing_strategy` as runtime availability signals, not as configuration promises.

Use `synrepo_readiness` when an agent needs a cheap preflight instead of the full orientation dashboard. It returns `graph`, `overlay`, `index`, `watch`, `reconcile`, `explain_hint`, and `edit_mode` status fields, plus capability rows with severity and next-action text. `details.branch_roots` reports configured read-only branch refs, local ref resolution, prepared snapshots, branch indexes, polling interval, and watch-derived monitoring state. It is read-only, never starts watch, never reconciles, and reports whether overlay writes and source edits were enabled for this MCP process.

Use `synrepo_resume_context` when an agent resumes stale work and needs a compact repo-scoped packet before asking the user to repeat context. It accepts optional `repo_root`, `limit` (default `10`, capped at `50`), `since_days` (default `14`, capped at `365`), `budget_tokens` (default `2000`, clamped by the normal MCP response limits), and `include_notes` (default `true`). It returns `schema_version: 1`, `packet_type: "repo_resume_context"`, `context_state`, `sections`, `detail_pointers`, and `omitted`. The packet is derived from current changed files, next actions, recent activity, explicit overlay note summaries, saved lesson summaries, validation commands, and context metrics. Notes and lessons are summary-only: id, target, status, confidence, updated time, and a short claim preview; lessons also include freshness. The tool is advisory and regeneratable. It does not store prompt logs, chat history, raw tool outputs, caller identity, response bodies, or generic session memory.
...
Saved repo lessons are a small operator-facing layer over advisory agent notes. `synrepo_lesson_search` and `synrepo_lesson_list` are read-only and available by default. `synrepo_lesson_add`, `synrepo_lesson_forget`, and `synrepo_lesson_verify` require `--allow-overlay-writes`. Lesson add accepts `claim`, optional `target_kind` (`repo`, `path`, `file`, `symbol`, `concept`, `test`, `card`, `note`), optional `target`, optional `ttl_seconds` capped at 365 days, `evidence`, `source_hashes`, `graph_revision`, `created_by`, and `confidence`. Responses return `lesson_id`, `target_kind`, `target`, `claim`, `status`, `freshness`, optional `expires_at`, `confidence`, `evidence`, `source_hashes`, `source_store: "overlay"`, and `advisory: true`. Lessons remain overlay-only, freshness-labeled, and advisory; they are not graph facts, generic session memory, or automatic prompt injection.
...
Use `synrepo_refactor_suggestions` when an agent or operator asks whether source files should be refactored or documented. It is read-only and defaults to `mode: "line_count"`, which returns deterministic facts for non-test source files over a physical-line threshold: graph file IDs, paths, language labels, line counts, symbol counts, modularity tags, and suggested follow-up MCP tools. `mode: "missing_docs"` reports non-test files with API symbols whose AST-extracted `doc_comment` is missing for wired tree-sitter languages with doc-comment extraction. JavaScript-family files only count ES module or CommonJS exports; TypeScript and TSX keep graph visibility behavior. Python only counts explicit API exposure through literal `__all__`, direct public definitions in package `__init__.py`, or simple sibling re-exports from package `__init__.py`. Responses include a `criteria` object that states whether the line-count threshold was applied; missing-doc symbol previews include `symbol_id` for precise follow-up. The response is labeled `source_store: "graph+filesystem"` because it combines graph metadata with current filesystem line counts; it does not generate edits, comments, or refactor plans.
...
`synrepo_readiness` returns a compact preflight: `graph: "ready|missing|error"`, `overlay: "ready|ready_empty|missing|error"`, `index: "ready|stale|missing"`, `watch: "active|inactive|starting|stale|error"`, `reconcile: "fresh|stale|missing|error"`, `explain_hint`, and `edit_mode: { overlay_writes, source_edits }`. `ready_empty` means `.synrepo/overlay/overlay.db` exists with empty schema tables and no advisory rows yet. When overlay commentary exists, `explain_hint` tells agents they can read it with `synrepo_explain budget=deep` or `synrepo_docs_search`, and whether refresh is unavailable because `overlay_writes=false`. The `details.capabilities` array mirrors the readiness matrix used by `synrepo_overview`, including the `branch-refs` capability. `details.branch_roots` has `{ configured, resolved, tracked, stale, missing, poll_seconds, monitored, refs }`; each ref row includes `ref_name`, `root_id`, optional `commit`, and `resolved`, `prepared`, `indexed`, and `tracked` booleans.

`synrepo_card` accepts either `target` or `targets`. `targets` batches up to 10 files or symbols under one read epoch and returns per-target errors so one missing card does not fail the whole batch. Deep card batches are capped at 3 targets. `FileCard` includes `root_id`, `is_primary_root`, and optional `root_kind`, `root_label`, `root_ref`, `root_commit`, and `editable`; `SymbolCard` includes the same root metadata plus `file_id` and `path`. Deep `SymbolCard` responses may include `source_body_state` and `member_outline`; large container symbols such as classes, traits, interfaces, and modules return an outline instead of a full body when the body would dominate the card budget, while leaf symbols still return exact source bodies at deep budget. When search returns duplicate relative paths from different roots, prefer the returned `file_id` or `suggested_card_requests` entry over a bare path target. Branch-ref cards are read-only and suppress Git working-tree edit assumptions. A single card with `budget_tokens` retries smaller budget tiers before marking truncation; batch calls also apply the token cap to the whole response and report omitted targets. Path-like card targets can return a degraded file stub with existence and Git status when repository prep fails. Mutating tools never degrade. Cards remain the current serialized packet for compiled code artifacts and task contexts; `synrepo_ask` is the high-level task-context front door over those existing packets.

Framework route bindings are graph facts when Synrepo observes clear framework syntax or route file conventions. Current extraction covers Rust Axum/Actix/Rocket-style routes, Python FastAPI/Flask/Django URL declarations, TypeScript/JavaScript Express, NestJS HTTP controllers, absolute React Router `<Route path>` bindings, React Router default `app/routes` file routes, Java/Kotlin Spring mappings, Go Gin/chi/gorilla-style handlers, ASP.NET attributes, Laravel routes/resources, Rails routes/resources, and SvelteKit `src/routes` page/server files. Route bindings are represented as `SymbolKind::Route` symbols and `References` edges to resolved handler symbols. They are source/path-observed graph facts, not inferred overlay content.
...
Output limits apply after every MCP tool handler. The default response cap is 4000 estimated tokens and the hard cap is 12000 estimated tokens. Before destructive trimming, the final clamp tries deterministic compaction for search-shaped responses and known large row arrays. Search-shaped payloads reuse the compact search representation; other recognized arrays keep routing identifiers and bounded string previews. If compaction is still over budget, the existing array/string truncation path runs. The response preserves valid JSON, attaches `context_accounting.truncation_applied = true`, reports omitted fields where possible, and marks row compaction with `response_omitted[].strategy = "row_compaction"`. There is no retrieval cache or follow-up retrieval tool; callers should rerun the original tool with narrower parameters or follow returned card targets when more detail is needed.

Input limits reject oversized or invalid parameters through `INVALID_PARAMETER`: unknown budget tiers, search queries over 512 characters, card batches over 10 targets, deep card batches over 3 targets, broad `output_mode: "cards"` searches, context packs without targets or a non-empty goal, note or lesson claims over 4000 characters, note or lesson evidence arrays over 32 entries, note or lesson source hash arrays over 32 entries, lesson `ttl_seconds` outside 1 to 31536000 seconds, anchored edit batches over 100 edits, anchored edit batches touching over 20 files, single edit payloads over 256 KiB, and total submitted edit text over 512 KiB.
...
Both edit tools accept optional `root_id` for worktree-safe path resolution. Omitted `root_id` preserves legacy primary-checkout behavior. Node-ID targets use the graph file's stored root and reject a mismatched requested root. Branch-ref roots reject prepare/apply with `INVALID_PARAMETER` because they are read-only snapshots; switch or materialize a worktree before editing. Prepare responses include `root_id` and `is_primary_root`; apply validates that every edit's prepared anchor state and path still belong to the selected root before writing.
...
synrepo orient
synrepo ask "review this module"
