...
2. **`src/structure/parse/language.rs`** — the single source of truth for per-language metadata. Update, in order:
   ...
   - `Language::import_query()` — must expose an `@import_ref` capture. Add `const <LANG>_IMPORT_QUERY` the same way. Fan-out forms (`use foo::{a, b};` in Rust, `from foo import a, b` in Python) are supported by capturing a path prefix plus a leaf: Rust uses `@use_path` + `@use_item` joined with `::`; Python uses `@import_ref` + `@import_name` joined with `.`. The extractor (`extract/mod.rs::extract_import_refs`) dispatches on which capture pair is present and emits one `ExtractedImportRef` per leaf. The resolver's per-leaf trim fallback handles cases where the leaf is a symbol inside the resolved module file rather than a nested module.
