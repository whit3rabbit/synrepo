...
cargo run -- embeddings build      # build/rebuild optional semantic vector index
cargo run -- embeddings clean [--apply] [--json]  # list/remove stale vector profile dirs + legacy flat index (dry run by default)
...
cargo run -- bench search --tasks 'benches/tasks/*.json' --mode both --json  # compare lexical vs hybrid search hit@5
...
3. `FileNodeId` is stable across renames within one discovery root. For new files it is derived from `(root_discriminant, path, first_seen_content_hash)` (`derive_file_id` in `pipeline/structural/ids.rs`) to prevent collisions between byte-identical files. For existing files the stored ID is always reused. Content-hash rename detection (stage 6) is root-scoped: a file moved to a new path with identical content preserves its `FileNodeId` and records the old path in `path_history`. Do not derive `FileNodeId` dynamically from current path.
...
### Source discovery and project layouts

- **Supported-language extensions are centralized.** `src/core/source_language.rs` maps extensions to stable language labels used by both substrate classification and parser dispatch. Adding or changing a wired tree-sitter language must update that registry and the classifier/parser agreement tests. Do not reintroduce separate extension maps in `substrate/classify.rs` or `structure/parse/language.rs`.
- **Project layout detection is advisory.** `src/core/project_layout.rs` and its submodules detect manifests and conventional roots for Flutter/Dart, Android, Rust, Go, Node, and Python. This must not mutate `.synrepo/config.toml` or hide platform folders. Configured `roots` remain the discovery boundary; detected roots can inform readiness hints, import resolution, entrypoints, and test-surface.
- **Android layout support lives under `core/project_layout/android.rs`.** It detects Gradle markers and `AndroidManifest.xml`, then reports existing Java/Kotlin main, test, and androidTest roots, including one-level Gradle modules. Manifest launcher detection handles `activity` and `activity-alias` with `MAIN` plus `LAUNCHER`; keep this parser conservative and dependency-free unless XML complexity forces a dedicated parser.
- **JVM import resolution is indexed, not per-import filesystem walking.** Java/Kotlin source roots are collected once in `pipeline/structural/stage4/context.rs`; `imports.rs` resolves Java/Kotlin imports against those roots, including Java-to-Kotlin imports. Keep this root-scoped and avoid cross-root imports.
- **Android card and test conventions are split by concern.** Launcher entrypoints are in `surface/card/compiler/entry_point/`; path conventions live in `util/test_paths.rs`; test-symbol fallback lives in `surface/card/compiler/test_surface.rs`. Update `tests/android_discovery.rs` when changing Android behavior end to end.
...
- **Semantic MCP behavior must stay local-only and optional.** Query-time routing/search may use embeddings only when `semantic-triage` is compiled, config enables it, and local model/vector artifacts already load. Use `ModelResolver::resolve_existing` or `load_embedding_index`; do not trigger downloads, rebuilds, or remote calls from MCP handlers, hooks, or `task-route` except the configured local Ollama embed endpoint. Deterministic safety guards in `task_route.rs` run before semantic intent matching, and missing assets must return `routing_strategy: "keyword_fallback"` or `semantic_available: false`.
- **Dashboard embeddings toggle is config-only.** The TUI Actions-tab `T` key may persist `enable_semantic_triage`, but it must not run reconcile automatically. Building vectors remains an explicit follow-up (`B` in the dashboard or `synrepo embeddings build`) because model download and embedding calls are opt-in costs.
- **Ollama embedding endpoint.** Local Ollama embeddings use `semantic_embedding_provider = "ollama"`, `semantic_model = "all-minilm"`, `embedding_dim = 384`, and `semantic_ollama_endpoint = "http://localhost:11434"`. Smoke test with `curl http://localhost:11434/api/embed -d '{"model":"all-minilm","input":["First sentence","Second sentence"]}'`; expected output has two 384-dimensional embeddings. Ollama is optional and local-only.
- **Vector index is profile-keyed, not flat.** Each index lives under `.synrepo/index/vectors/<blake3-prefix>-<label>/index.bin`, keyed on `(provider, model, dim, precision, chunk_chars, normalizer_version)`. Changing any field in that tuple produces a different profile key, so model/dim/precision swaps build a new index side by side with the old one rather than invalidating it silently. The `INDEX_FORMAT_VERSION` constant in `src/substrate/embedding/index/persistence.rs` is currently 6; v5 and earlier refuse to load (fail-closed). See `docs/SCHEMA.md` § Embedding index for the on-disk header.
- **Instruction-tuned models need a query prefix.** `snowflake-arctic-embed-xs` and any future instruction-tuned model must apply its instruction prompt at query time only — documents never get the prefix. The plumbing lives on `OnnxModelResolution` / `OllamaModelResolution` (`query_prefix: Option<String>`) and is consumed by `EmbeddingSession::embed_query`, which the index routes through for query-time embedding. Documents still go through `EmbeddingSession::embed` (no prefix). Adding a new instruction-tuned model without setting `query_prefix` will silently produce wrong vectors.
...
- **`synrepo_refactor_suggestions` JSON is a public MCP contract.** `mode=line_count` is the default; `mode=missing_docs` reports active public graph symbols whose stored AST-extracted `doc_comment` is missing. Keep the flat candidate shape backward-compatible, keep missing-doc previews bounded, include `symbol_id` for follow-up, and update `docs/SCHEMA.md`, `docs/MCP.md`, `src/surface/mcp/README.md`, and MCP schema/contract tests when changing this shape. This surface reads graph facts plus filesystem line counts; it must not read overlay commentary or generate docs.
...
- **`signature` and `doc_comment` are populated** for all wired tree-sitter source languages when their language-specific doc syntax is present: Rust `///`, Python docstrings, TypeScript/TSX/JavaScript/Java/Kotlin/PHP `/** */`, Go contiguous `//` or `/* */`, C#/Swift/Dart `///` or `/** */`, C/C++ Doxygen-style `///`, `//!`, `/**`, or `/*!`, and Ruby contiguous `#`. Missing-doc suggestions must only target languages whose parser currently emits `doc_comment`.
