...
- **`src/structure/parse/extract/` is a sub-module directory** (`mod.rs` ~363 lines, `qualname.rs` ~88 lines) — do not add more code to `mod.rs` without splitting further. **Over-limit (split before adding):**
  ...
  - `src/tui/app/mod.rs` (622) — prefer a new `src/tui/app/` submodule (see the `impl AppState` split gotcha below) over growing this further
...
- **`src/tui/app/` sub-modules can own `impl AppState` blocks for feature-specific state and handlers** (see `synthesis_picker.rs` for the folder-picker modal). Prefer a new submodule to growing `app/mod.rs` when wiring a new dashboard feature: keep the `AppState` fields on the struct in `mod.rs`, but put the feature's methods, helpers, and tests in the submodule.
