...
- **Semantic MCP behavior must stay local-only and optional.** Query-time routing/search may use embeddings only when `semantic-triage` is compiled, config enables it, and local model/vector artifacts already load. Use `ModelResolver::resolve_existing` or `load_embedding_index`; do not trigger downloads or remote calls from MCP handlers, hooks, or `task-route` except the configured local Ollama embed endpoint. Deterministic safety guards in `task_route.rs` run before semantic intent matching, and missing assets must return `routing_strategy: "keyword_fallback"` or `semantic_available: false`.
