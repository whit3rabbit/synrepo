...
- **`src/structure/parse/extract/` is a sub-module directory** (`mod.rs` ~388 lines, `qualname.rs` ~59 lines) — do not add more code to `mod.rs` without splitting further. **Over-limit (split before adding):** `src/pipeline/repair/report.rs` (648), `src/store/sqlite/ops.rs` (590), `src/pipeline/repair/sync.rs` (568), `src/structure/identity.rs` (509), `src/pipeline/structural/stages.rs` (486). **Watchlist (approaching limit):** `src/surface/card/types.rs` (393), `src/pipeline/git/mod.rs` (382), `src/structure/graph/store.rs` (347), `src/surface/mcp/primitives.rs` (336), `src/bin/cli_support/commands/mcp.rs` (326), `src/structure/prose.rs` (325), `src/store/compatibility/evaluate/mod.rs` (320), `src/pipeline/diagnostics.rs` (310).
