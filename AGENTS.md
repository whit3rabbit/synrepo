...
- **Dashboard embeddings toggle is config-only.** The TUI Actions-tab `T` key may persist `enable_semantic_triage`, but it must not run reconcile automatically. Enabling vectors remains an explicit follow-up (`R` in the dashboard or `synrepo reconcile`) because model download and embedding calls are opt-in costs.
