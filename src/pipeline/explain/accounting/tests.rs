use super::super::telemetry::{ExplainEvent, ExplainTarget, TokenUsage};
use super::*;
...
fn completed(call_id: u64, usage: TokenUsage) -> ExplainEvent {
...
fn completed_event_appends_and_updates_totals() {
    ...
    let rec: ExplainCallRecord = serde_json::from_str(log.lines().next().unwrap()).unwrap();
...
fn unknown_model_flips_any_unpriced_bit_and_records_null_cost() {
    let dir = tempdir().unwrap();
    let event = ExplainEvent::CallCompleted {
        call_id: 1,
        provider: "openai",
...
fn failed_event_records_error_and_bumps_failures() {
    let dir = tempdir().unwrap();
    let event = ExplainEvent::CallFailed {
        call_id: 1,
        provider: "openai",
        model: "gpt-4o-mini".to_string(),
        target: file_target(1),
...
fn call_started_events_are_not_logged() {
    let dir = tempdir().unwrap();
    let event = ExplainEvent::CallStarted {
        call_id: 1,
        provider: "openai",
        model: "gpt-4o-mini".to_string(),
        target: file_target(1),
