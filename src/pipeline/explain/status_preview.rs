...
use super::{describe_active_provider, ActiveProvider, ExplainStatus};

/// Number of sample targets kept per queued-work group.
pub const SAMPLE_LIMIT_PER_GROUP: usize = 3;

/// One queued-work group within a explain preview.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ExplainPreviewGroup {
...
/// Display-ready summary of what `synrepo explain` would do for a scope.
#[derive(Clone, Debug, Eq, PartialEq)]
pub struct ExplainPreview {
    ...
    pub refresh: ExplainPreviewGroup,
    /// Files lacking commentary that would be seeded.
    pub file_seeds: ExplainPreviewGroup,
    /// Symbols lacking commentary that would be seeded.
    pub symbol_seeds: ExplainPreviewGroup,
...
/// Build a queued-work preview for the requested explain scope.
pub fn build_explain_preview(
    repo_root: &Path,
    paths: Vec<String>,
    changed: bool,
) -> anyhow::Result<ExplainPreview> {
    let config = Config::load(repo_root).map_err(|e| {
        anyhow::anyhow!("explain status: not initialized — run `synrepo init` first ({e})")
    })?;
    ...
    let plan = load_commentary_work_plan(&synrepo_dir, scope.as_deref())
        .map_err(|e| anyhow::anyhow!("explain status: cannot plan commentary work ({e})"))?;
...
fn build_group(label: &'static str, items: &[CommentaryWorkItem]) -> ExplainPreviewGroup {
...
fn summary_line(
    ...
    if plan_is_empty {
        format!(
            "checked {scoped_file_count} file(s) and {scoped_symbol_count} symbol(s) in scope. nothing currently needs explain for this scope."
        )
    } else {
        format!(
            "checked {scoped_file_count} file(s) and {scoped_symbol_count} symbol(s) in scope. {max_target_count} target(s) would be reconsidered if you run `synrepo explain` now."
        )
    }
}
...
fn compute_scope(
    ...
    if changed {
        ...
        let insights = analyze_recent_history(&context, 50, 50)
            .map_err(|e| anyhow::anyhow!("explain status: cannot sample git history ({e})"))?;
...
fn api_status_line(status: &ExplainStatus, provider: &str) -> String {
    match status {
        ExplainStatus::Enabled => format!(
        ...
        ExplainStatus::Disabled => {
            "no, explain is disabled so no provider requests will be made".to_string()
        }
        ExplainStatus::DisabledKeyDetected { env_var } => {
            format!("no, explain is disabled even though ${env_var} is set")
        }
    }
}
