...
fn stage4_emits_calls_edge_for_cross_file_rust_call() {
    // utils.rs defines `helper`. lib.rs calls `utils::helper()`.
    // Stage 4 should emit a Calls edge from lib.rs (file) to helper (symbol).
    let repo = tempdir().unwrap();
    fs::create_dir_all(repo.path().join("src")).unwrap();
...
fn stage4_emits_imports_edge_for_typescript_relative_import() {
    // main.ts imports utils.ts via './utils'.
    // Stage 4 should emit an Imports edge from main.ts to utils.ts.
    let repo = tempdir().unwrap();
    fs::create_dir_all(repo.path().join("src")).unwrap();
