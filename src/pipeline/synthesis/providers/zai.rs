//! Z.ai (Zhipu / GLM) synthesis provider.
//!
...
/// Default Z.ai model for synthesis (GLM-4.6 is the current flagship, same
...
const PASS_ID: &str = "commentary-v1-zai";
const CROSS_LINK_PASS_ID: &str = "cross-link-v1-zai";

/// Z.ai-backed commentary generator.
pub struct ZaiCommentaryGenerator {
    api_key: String,
    model: String,
...
impl CommentaryGenerator for ZaiCommentaryGenerator {
...
pub struct ZaiCrossLinkGenerator {
    api_key: String,
    model: String,
    max_tokens_per_call: u32,
...
impl CrossLinkGenerator for ZaiCrossLinkGenerator {
...
mod tests {
    ...
    fn new_constructs_without_panicking() {
        let gen =
            ZaiCommentaryGenerator::new("fake-key".to_string(), "test-model".to_string(), 5000);
