//! Search and enrich synthesized commentary docs for MCP callers.
...
pub struct CommentaryDocHit {
    ...
    /// Repo-relative synthesized-doc path under `.synrepo/`.
    pub path: String,
...
fn enrich_hits(
    ...
    for hit in matches {
        ...
        let path = repo_relative_doc_path(node_id)
            .unwrap_or_else(|| std::path::PathBuf::from(".synrepo/synthesis-docs").join(&hit.path))
            .to_string_lossy()
            .into_owned();
