...
use super::super::telemetry::{Outcome, SynthesisEvent, UsageSource};
use super::record::record_for_event;
use super::types::{SynthesisCallRecord, SynthesisTotals};
...
/// `synthesis-log.jsonl.<rfc3339>.bak` so nothing is lost.
...
pub fn load_totals(synrepo_dir: &Path) -> std::io::Result<Option<SynthesisTotals>> {
    match std::fs::read(totals_path(synrepo_dir)) {
        ...
        Err(error) => Err(error),
        Ok(body) => serde_json::from_slice::<SynthesisTotals>(&body)
            .map(Some)
