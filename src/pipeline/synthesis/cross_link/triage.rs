...
//!
//! The filter walks configured concept directories (via the graph) and pairs
//! their mentions of identifiers with the symbols whose qualified names
//! match. A BFS over observed edges bounds the graph distance per pair so an
//! unrelated concept cannot pull in a symbol halfway across the repo. Only
//! pairs that survive both filters are handed to the LLM.
//!
//! The filter is deliberately conservative: a zero-cost miss is fine because
//! a slower pass can surface it later, but the LLM must never see unbounded
//! fan-out.
...
use crate::{config::Config, core::ids::ConceptNodeId};
...
use super::{CandidatePair, TriageSource};
...
/// Split a prose string into candidate identifier tokens. Keeps alphanumeric
/// and underscore runs; drops everything else so `` `fn authenticate()` ``
/// yields `fn` + `authenticate`. The caller's MIN_IDENT_LEN check filters
/// the short tokens.
...
fn build_symbol_index(
    symbols: &[(SymbolNodeId, crate::core::ids::FileNodeId, String)],
...
pub fn all_candidates(
    ...
    let deterministic = candidate_pairs(graph, scope)?;
