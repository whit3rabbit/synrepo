//! Folder-picker sub-view for the Synthesis tab. Enumerates top-level repo
//! directories, seeds default selection from a heuristic (≥1 parser-supported
//! file) or a persisted prior selection at
//! `.synrepo/state/synthesis-scope.json`, and exposes the state + key
...
const SCOPE_FILE: &str = ".synrepo/state/synthesis-scope.json";
...
/// On-disk schema for `.synrepo/state/synthesis-scope.json`.
...
pub fn save_scope(repo_root: &Path, paths: &[String]) -> crate::Result<()> {
    ...
    let text = serde_json::to_string_pretty(&state)
        .map_err(|e| crate::Error::Config(format!("synthesis-scope serialize: {e}")))?;
    fs::write(&path, text)?;
    Ok(())
}
