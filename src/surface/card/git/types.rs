//! Data types for git-derived card projections.
...
mod tests {
    use super::{
        FileGitCoChange, FileGitCommit, FileGitIntelligence, FileGitOwnership,
        LastChangeGranularity,
    };
    use crate::pipeline::{
        git::{GitCommitSummary, GitIntelligenceReadiness},
        git_intelligence::{
    ...
    #[test]
    fn file_git_intelligence_from_borrowed_matches_owned() {
        let insights = GitPathHistoryInsights {
            path: "src/lib.rs".to_string(),
            status: GitIntelligenceStatus {
                source_revision: "deadbeef".to_string(),
                requested_commit_depth: 8,
        ...
        assert_eq!(
            FileGitIntelligence::from(&insights),
            FileGitIntelligence::from(insights)
        );
    }
}
