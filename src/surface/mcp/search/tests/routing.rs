...
use super::super::{default_edit_limit, handle_where_to_edit};
...
#[test]
fn where_to_edit_routes_lifecycle_review_before_phrase_noise() {
    let (_dir, state) = make_routing_state();
    let value: serde_json::Value = serde_json::from_str(&handle_where_to_edit(
        &state,
        "Review likely memory/resource leaks and uncaught error handling risks in long-lived server, goroutine, PTY/process, database, and timer lifecycle code".to_string(),
        default_edit_limit(),
        None,
    ))
    .unwrap();
    ...
    assert!(attempts
        .iter()
        .any(|attempt| attempt["query"] == "goroutine"));
    assert!(attempts
        .iter()
        .any(|attempt| attempt["query"] == "QueryContext"));
    assert!(!attempts
        .iter()
        .any(|attempt| attempt["query"] == "review_likelys"));

    let recommendations = value["recommended_next_queries"].as_array().unwrap();
    assert!(recommendations.iter().any(|query| query == "goroutine"));
    assert!(!recommendations.iter().any(|query| query == "read_resource"));
}
