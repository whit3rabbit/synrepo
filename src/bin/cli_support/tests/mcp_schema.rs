...
fn refactor_suggestions_params_and_docs_are_listed() {
    ...
    assert!(source.contains("pub mode: RefactorSuggestionMode"));
    ...
    let schema = schemars::schema_for!(
        synrepo::surface::mcp::refactor_suggestions::RefactorSuggestionsParams
    );
    let schema_json = serde_json::to_value(schema).expect("schema serializes");
    let serialized = serde_json::to_string(&schema_json).unwrap();
    assert!(
        serialized.contains("\"mode\""),
        "refactor suggestions schema must expose mode: {schema_json}"
    );
