...
/// `~/.synrepo/config.toml` based on the synthesis choice captured by the
...
pub(crate) fn step_apply_synthesis(
    ...
    use synrepo::tui::wizard::setup::synthesis::{CloudCredentialSource, SynthesisChoice};
    ...
    let local_synthesis = ensure_table(&mut local_doc, &local_path, "synthesis")?;
    local_synthesis.set_implicit(false);
    local_synthesis.insert("enabled", Item::Value(TomlValue::from(true)));
    for field in PROVIDER_KEY_FIELDS {
        local_synthesis.remove(field);
    }
    local_synthesis.remove("local_endpoint");
    local_synthesis.remove("local_preset");

    match choice {
        SynthesisChoice::Cloud { provider, .. } => {
            local_synthesis.insert(
                "provider",
                Item::Value(TomlValue::from(provider.config_value())),
            );
        }
        SynthesisChoice::Local { .. } => {
            local_synthesis.insert("provider", Item::Value(TomlValue::from("local")));
        }
    }
    ...
    match choice {
        SynthesisChoice::Cloud {
            ...
            let api_key = api_key
                ...
                .ok_or_else(|| {
                    anyhow!(
                        "refusing to persist {} synthesis config: entered API key is missing",
                        provider.config_value()
                    )
                })?;
            let global_path = global_config_path()?;
            let mut global_doc = load_toml_document(&global_path)?;
            let global_synthesis = ensure_table(&mut global_doc, &global_path, "synthesis")?;
            global_synthesis.set_implicit(false);
            global_synthesis.insert(
        ...
        SynthesisChoice::Local { preset, endpoint } => {
            ...
            write_toml_document(&global_path, &global_doc)?;
            println!(
                "  Saved local synthesis endpoint in {}",
                global_path.display()
            );
        }
        _ => {}
    ...
    println!(
        "  Wrote repo-local [synthesis] block to {} (provider: {})",
        local_path.display(),
        match choice {
            SynthesisChoice::Cloud { provider, .. } => provider.config_value(),
