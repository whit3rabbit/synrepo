...
impl SynrepoServer {
    ...
    async fn synrepo_overview(&self, Parameters(params): Parameters<search::RepoRootParams>) -> String {
        self.with_tool_state("synrepo_overview", params.repo_root.clone(), |state| search::handle_overview(&state))
    }
    ...
    async fn synrepo_node(&self, Parameters(params): Parameters<primitives::NodeParams>) -> String {
        self.with_tool_state("synrepo_node", params.repo_root.clone(), |state| primitives::handle_node(&state, params.id))
    }
    ...
    async fn synrepo_overlay(&self, Parameters(params): Parameters<primitives::OverlayParams>) -> String {
        self.with_tool_state("synrepo_overlay", params.repo_root.clone(), |state| primitives::handle_overlay(&state, params.id))
    }
    ...
    async fn synrepo_provenance(&self, Parameters(params): Parameters<primitives::ProvenanceParams>) -> String {
        self.with_tool_state("synrepo_provenance", params.repo_root.clone(), |state| primitives::handle_provenance(&state, params.id))
    }
    ...
    async fn synrepo_where_to_edit(&self, Parameters(params): Parameters<search::WhereToEditParams>) -> String {
        self.with_tool_state("synrepo_where_to_edit", params.repo_root.clone(), |state| search::handle_where_to_edit(&state, params.task, params.limit, params.budget_tokens))
    }
    ...
    async fn synrepo_change_impact(&self, Parameters(params): Parameters<search::ChangeImpactParams>) -> String {
        self.with_tool_state("synrepo_change_impact", params.repo_root.clone(), |state| search::handle_change_impact(&state, params.target))
    }
    ...
    async fn synrepo_entrypoints(&self, Parameters(params): Parameters<cards::EntrypointsParams>) -> String {
        self.with_tool_state("synrepo_entrypoints", params.repo_root.clone(), |state| cards::handle_entrypoints(&state, params.scope, params.budget, params.budget_tokens))
    }
    ...
    async fn synrepo_note_add(&self, Parameters(params): Parameters<notes::NoteAddParams>) -> String {
        let repo_root = params.repo_root.clone();
        self.with_tool_state("synrepo_note_add", repo_root, |state| notes::handle_note_add(&state, params))
    }
    ...
    async fn synrepo_note_link(&self, Parameters(params): Parameters<notes::NoteLinkParams>) -> String {
        let repo_root = params.repo_root.clone();
        self.with_tool_state("synrepo_note_link", repo_root, |state| notes::handle_note_link(&state, params))
    }
    ...
    async fn synrepo_note_supersede(&self, Parameters(params): Parameters<notes::NoteSupersedeParams>) -> String {
        let repo_root = params.repo_root.clone();
        self.with_tool_state("synrepo_note_supersede", repo_root, |state| notes::handle_note_supersede(&state, params))
    }
    ...
    async fn synrepo_note_forget(&self, Parameters(params): Parameters<notes::NoteForgetParams>) -> String {
        let repo_root = params.repo_root.clone();
        self.with_tool_state("synrepo_note_forget", repo_root, |state| notes::handle_note_forget(&state, params))
    }
    ...
    async fn synrepo_note_verify(&self, Parameters(params): Parameters<notes::NoteVerifyParams>) -> String {
        let repo_root = params.repo_root.clone();
        self.with_tool_state("synrepo_note_verify", repo_root, |state| notes::handle_note_verify(&state, params))
    }
    ...
    async fn synrepo_notes(&self, Parameters(params): Parameters<notes::NotesParams>) -> String {
        let repo_root = params.repo_root.clone();
        self.with_tool_state("synrepo_notes", repo_root, |state| notes::handle_notes(&state, params))
    }
    ...
    async fn synrepo_module_card(&self, Parameters(params): Parameters<cards::ModuleCardParams>) -> String {
        self.with_tool_state("synrepo_module_card", params.repo_root.clone(), |state| cards::handle_module_card(&state, params.path, params.budget, params.budget_tokens))
    }
    ...
    async fn synrepo_public_api(&self, Parameters(params): Parameters<cards::PublicAPICardParams>) -> String {
        self.with_tool_state("synrepo_public_api", params.repo_root.clone(), |state| cards::handle_public_api(&state, params.path, params.budget, params.budget_tokens))
    }
    ...
    async fn synrepo_minimum_context(&self, Parameters(params): Parameters<cards::MinimumContextParams>) -> String {
        self.with_tool_state("synrepo_minimum_context", params.repo_root.clone(), |state| {
    ...
    async fn synrepo_call_path(&self, Parameters(params): Parameters<cards::CallPathParams>) -> String {
        self.with_tool_state("synrepo_call_path", params.repo_root.clone(), |state| cards::handle_call_path(&state, params.target, params.budget, params.budget_tokens))
    }
    ...
    async fn synrepo_test_surface(&self, Parameters(params): Parameters<cards::TestSurfaceParams>) -> String {
        self.with_tool_state("synrepo_test_surface", params.repo_root.clone(), |state| cards::handle_test_surface(&state, params.scope, params.budget, params.budget_tokens))
    }
    ...
    async fn synrepo_change_risk(&self, Parameters(params): Parameters<cards::ChangeRiskParams>) -> String {
        self.with_tool_state("synrepo_change_risk", params.repo_root.clone(), |state| cards::handle_change_risk(&state, params.target, params.budget, params.budget_tokens))
    }
    ...
    async fn synrepo_orient(&self, Parameters(params): Parameters<search::RepoRootParams>) -> String {
        self.with_tool_state("synrepo_orient", params.repo_root.clone(), |state| {
    ...
    async fn synrepo_find(&self, Parameters(params): Parameters<search::WhereToEditParams>) -> String {
        self.with_tool_state("synrepo_find", params.repo_root.clone(), |state| {
    ...
    async fn synrepo_explain(&self, Parameters(params): Parameters<cards::CardParams>) -> String {
        self.with_tool_state("synrepo_explain", params.repo_root.clone(), |state| {
    ...
    async fn synrepo_impact(&self, Parameters(params): Parameters<cards::ChangeRiskParams>) -> String {
        self.with_tool_state("synrepo_impact", params.repo_root.clone(), |state| {
    ...
    async fn synrepo_risks(&self, Parameters(params): Parameters<cards::ChangeRiskParams>) -> String {
        self.with_tool_state("synrepo_risks", params.repo_root.clone(), |state| {
    ...
    async fn synrepo_tests(&self, Parameters(params): Parameters<cards::TestSurfaceParams>) -> String {
        self.with_tool_state("synrepo_tests", params.repo_root.clone(), |state| {
    ...
    async fn synrepo_changed(&self, Parameters(params): Parameters<search::RepoRootParams>) -> String {
        self.with_tool_state("synrepo_changed", params.repo_root.clone(), |state| {
    ...
    async fn synrepo_refresh_commentary(&self, Parameters(params): Parameters<cards::RefreshCommentaryParams>) -> String {
        self.with_tool_state("synrepo_refresh_commentary", params.repo_root.clone(), |state| cards::handle_refresh_commentary(&state, params.target))
    }
    ...
    async fn synrepo_findings(&self, Parameters(params): Parameters<audit::FindingsParams>) -> String {
        self.with_tool_state("synrepo_findings", params.repo_root.clone(), |state| audit::handle_findings(&state.repo_root, params.node_id, params.kind, params.freshness, params.limit))
    }
    ...
    async fn synrepo_recent_activity(&self, Parameters(params): Parameters<audit::RecentActivityParams>) -> String {
        self.with_tool_state("synrepo_recent_activity", params.repo_root.clone(), |state| audit::handle_recent_activity(&state, params.kinds, params.limit, params.since))
    }
    ...
    async fn synrepo_prepare_edit_context(&self, Parameters(params): Parameters<edits::PrepareEditContextParams>) -> String {
        let repo_root = params.repo_root.clone();
        self.with_tool_state("synrepo_prepare_edit_context", repo_root, |state| edits::handle_prepare_edit_context(&state, params))
    }
    ...
    async fn synrepo_apply_anchor_edits(&self, Parameters(params): Parameters<edits::ApplyAnchorEditsParams>) -> String {
        let repo_root = params.repo_root.clone();
        self.with_tool_state("synrepo_apply_anchor_edits", repo_root, |state| edits::handle_apply_anchor_edits(&state, params))
    }
    ...
    async fn synrepo_next_actions(&self, Parameters(params): Parameters<audit::NextActionsParams>) -> String {
        ...
        self.with_tool_state("synrepo_next_actions", params.repo_root.clone(), |state| match collect_handoffs(&state.repo_root, &state.config, &request) {
