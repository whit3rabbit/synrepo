...
- [x] 6.3 Add `retired_observations_findings()` to `src/pipeline/repair/report.rs`: count retired symbols/edges, report compaction recommendation when count exceeds threshold
- [x] 6.4 Wire `retired_observations_findings` into `check` output
- [x] 6.5 Wire `CompactRetired` action into `sync` dispatch
...
- [x] 7.2 Manual: `cargo run -- init` on fixture repo, edit a file, re-init, verify `FileNodeId` persists via `synrepo node <id>`
- [x] 7.3 Manual: `synrepo check --json` after edit shows non-zero drift, file_id unchanged
- [x] 7.4 Manual: delete a symbol, verify `retired_at_rev` set, `active_edges()` excludes its edges
- [x] 7.5 Update CLAUDE.md/AGENTS.md: document new invariant (FileNodeId stable across content edits), new config field, compaction behavior
- [x] 7.6 Update ROADMAP.md: add graph-lifecycle-v1 entry
