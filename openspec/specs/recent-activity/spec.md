## Purpose
Define the behavioral contract for synrepo's bounded recent-activity surface: the `synrepo_recent_activity` MCP tool and the `synrepo status --recent` CLI flag.
